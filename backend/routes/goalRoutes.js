...
// AI helper function - uses Cohere v2/chat API
const callOpenAI = async (prompt, maxTokens = 500) => {
  ...
  for (const model of models) {
    try {
      console.log(`[goalRoutes] Calling Cohere v2/chat with model: ${model}`);
      ...
      if (result) {
        console.log(`[goalRoutes] Cohere ${model} success!`);
        return result;
      }
    } catch (error) {
      console.error(`[goalRoutes] Cohere ${model} error:`, error.response?.status, error.response?.data?.message || error.message);
    }
  }
